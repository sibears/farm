...
impl FlagRepo for PostgresFlagRepo {
    ...
    fn save(&self, flag_arg: &NewFlag) -> Result<usize, Self::FlagRepoError> {
        use crate::schema::flags::dsl::*;
        let mut conn = self.conn.lock().unwrap();
        diesel::insert_into(flags).values(flag_arg).execute(&mut *conn)
    }

    fn save_all(&self, flags_arg: &[NewFlag]) -> Result<usize, Self::FlagRepoError> {
        use crate::schema::flags::dsl::*;
        let mut conn = self.conn.lock().unwrap();
    ...
    fn update_all(&self, flags_arg: &[Flag]) -> Result<usize, Self::FlagRepoError> {
        ...
        for flag in flags_arg {
            total_updated += diesel::update(crate::schema::flags::dsl::flags.filter(crate::schema::flags::dsl::id.eq(flag.id)))
                .set(flag)
                .execute(&mut *conn)?;
        }
        Ok(total_updated)
    ...
    fn get_limit(&self, limit: i64) -> Result<Vec<Flag>, Self::FlagRepoError> {
        use crate::schema::flags::dsl::*;
        let mut conn = self.conn.lock().unwrap();
        flags.limit(limit).load::<Flag>(&mut *conn)
    }
