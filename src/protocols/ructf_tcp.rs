                ...
                match stream.read(&mut buf) {
                    Ok(_) => {
                        ...
                        for mut flag in queue_flags {
                            ...
                            match stream.read(&mut buf) {
                                Ok(_) => {
                                    let answ = CStr::from_bytes_until_nul(&buf).unwrap().to_string_lossy();
                                    let msg = answ.replace(format!("[{}] ", flag.flag).as_str(), "");
                                    flag.checksystem_response = Some(msg.clone().into());
                                    for (status, key_words) in &responses {
                                        if key_words.iter().any(|word| msg.to_lowercase().contains(word)) {
